"""Training script for Masked Autoencoder (MAE) on STL-10."""
...
# Allow running as `python scripts/train_mae.py` from project root
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train MAE on STL-10")
    # Model
    ...
    # Logging & checkpoints
    parser.add_argument("--log-dir", type=str, default="results/mae")
...
def main():
    ...
    print(f"  Decoder:       dim={args.decoder_dim}, depth={args.decoder_depth}, heads={args.decoder_heads}")
    print(f"  Mask ratio:    {args.mask_ratio}")
    print(f"  Patch size:    {args.patch_size}x{args.patch_size} ({model.num_patches} patches)")
